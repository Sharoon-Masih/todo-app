...
let anslist;
let tasklist: string[] = ["todo1", "todo2", "todo3", "todo4"];
let ans: { newTodo: string };
let remove: { toRemove: string };
let completed: { done: string[] };
let newtask: string[] = [];
let newtask2: string[] = [];
...
do {
    ...
    if (anslist.task === `${chalk.blue.italic.bold("Remove todo")}`) {
        ...
        isRemove = true;
        if (!isDone) {
            let removed: string | undefined = tasklist.find((val) => val === remove.toRemove)//yaha find ka method iss lia use kia hai bcuz find ka method puray array ma jo be element pehla match hogya user ka answer say wo return hojayega in string form.bcuz agr filter use krtay hain toh wo array return krta hai iss lia find use kia hai bcuz indexof ma hum string pss krskty hain array nahi.
            ...
            if (typeof removed === "string") {  //here we do narrowing bcuz 'removed' have union type string and undefined so thats why ab indexof() ka method toh string value hi legana so iss lia agr removed ki type string hogi toh agy proceed hoga else "undefined". 
                let index: number = tasklist.indexOf(removed)
            ...
            let removed: string | undefined = newtask2.find((val) => val === remove.toRemove)//yaha find ka method iss lia use kia hai bcuz find ka method puray array ma jo be element pehla match hogya user ka answer say wo return hojayega in string form.bcuz agr filter use krtay hain toh wo array return krta hai iss lia find use kia hai bcuz indexof ma hum string pss krskty hain array nahi.
            console.log(chalk.yellow.italic.bold("task removed:"), removed);

            if (typeof removed === "string") {  //here we do narrowing bcuz 'removed' have union type string and undefined so thats why ab indexof() ka method toh string value hi legana so iss lia agr removed ki type string hogi toh agy proceed hoga else "undefined". 
                let index: number = newtask2.indexOf(removed)
        ...
        let Remove: string[] = completed.done
        console.log(Remove);
        let i: number;
